...
"""
Streaming Transformer with CTC Loss for IMU-based Digit Recognition
...
class Config:
    ...
    # Data dimensions
    MAX_LEN = 138          # Maximum sequence length (after padding and waveletTransform)
...
def train_model(model, optimizer, train_dataset, epochs=None):
    ...
    for epoch in range(epochs):
        ...
        for step, (x, y, input_len, label_len) in enumerate(train_dataset):
            loss = train_step(model, optimizer, x, y, input_len, label_len)
...
def main():
    ...
    print("=" * 60)
    print("Streaming Transformer with CTC Loss for IMU Digit Recognition")
    print("=" * 60)
